...
:: Optional: copy server cert/key for 192.168.100.100 to Laragon SSL folder
set "SSL_DIR=C:\laragon\etc\ssl"
if exist "%SSL_DIR%" (
  if exist "%~dp0labbahasa-192.168.100.100.crt" copy /Y "%~dp0labbahasa-192.168.100.100.crt" "%SSL_DIR%" >nul
  if exist "%~dp0labbahasa-192.168.100.100.key" copy /Y "%~dp0labbahasa-192.168.100.100.key" "%SSL_DIR%" >nul
)

echo Selesai untuk IP 192.168.100.100. Silakan restart Chrome/Edge.
pause
